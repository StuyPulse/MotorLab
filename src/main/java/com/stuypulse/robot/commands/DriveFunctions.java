...
public class DriveFunctions {
    ...
    public static void bestestControl(Motor left, Motor right) {
        ...
        PIDController pid = new PIDController(Settings.Romi.PID.kP, Settings.Romi.PID.kI,Settings.Romi.PID.kD);

        //XXX:FeedForwardController is not used here because it is built into SimRomis
        //FeedForwardController feedForward = new FeedForwardController(Settings.Romi.Feedforward.kV, Settings.Romi.Feedforward.kS, Settings.Romi.Feedforward.kA);

        error = (setpoint - left.getDistance() + setpoint + right.getDistance()) / 2;
        double output = pid.getOutput(error) /*feedForward.getOutput(setpoint)*/;
